...
const X_COOKIE_DOMAIN: &str = "x.com";
const X_REFERER: &str = "https://x.com/";
const X_ORIGIN: &str = "https://x.com";
...
impl TwitterAuth for TwitterUserAuth {
    async fn install_headers(&self, headers: &mut HeaderMap) -> Result<()> {
        let cookie_jar = self.cookie_jar.lock().await;
        let cookies: Vec<_> = cookie_jar.iter().collect();
        let has_auth_token = cookies.iter().any(|c| c.name() == "auth_token");
        if !cookies.is_empty() {
        ...
        headers.insert("User-Agent", HeaderValue::from_static(X_USER_AGENT));
        headers.insert("Referer", HeaderValue::from_static(X_REFERER));
        headers.insert("Origin", HeaderValue::from_static(X_ORIGIN));
        ...
        headers.insert(
            "x-twitter-auth-type",
            HeaderValue::from_static(if has_auth_token {
                "OAuth2Session"
            } else {
                "OAuth2Client"
            }),
