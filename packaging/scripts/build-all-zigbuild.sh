  ...
  (
    ...
    if [[ "$target" == *"windows"* ]]; then
      if command -v zip >/dev/null 2>&1; then
        ...
        tar -czf "../${archive_name}.tar.gz" "${PACKAGE_NAME}.exe"
      fi
    else
...
build_target() {
  ...
  if cargo zigbuild --release --target "$target" -p rzn_tools_cli --features full; then
...
main() {
  print_header "Building rzn-tools v${VERSION} release artifacts with zigbuild"
  check_prerequisites
  prepare_build_dir
  validate_assets
  create_workflow_bundle
