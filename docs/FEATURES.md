...
- `server-full`: Portable API/HTTP connectors for server and downstream distributions. It does not read local browser profiles, include Rookie or publicsuffix, or enable the legacy `x-browser` connector.
- `desktop-full`: `server-full` plus `x-browser` and `browser-cookie-import` for an explicit, advanced local browser-profile import path.
